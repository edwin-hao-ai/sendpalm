...
### 11.3 Lessons learned (Session 2026-08-18 — IMAP chunk-walking bug, Imbox tabs, view-health audit fixes)

- **Sparse mailboxes need UIDNEXT-based termination, not "partial chunk"** — the Feishu account has ~3900 messages spread across UIDs 1..7228 (density ~0.54 msgs/UID). The old `messages.len() < MAX_PER_TICK` termination fired after the first chunk because every subsequent chunk was also partial. Read `mailbox.uid_next` from the SELECT response and loop until `cursor >= uid_next - 1`. Keep the legacy "stop on partial chunk" fallback for servers that don't expose UIDNEXT. `app/src-tauri/src/services/sync_loop.rs` (`should_continue_after_chunk`).
- **Empty chunk ≠ end of mailbox** — when the requested 200-UID range returns 0 messages (all deleted/expired), the loop should advance cursor by `MAX_PER_TICK` and try the next range, not break. The previous behaviour made any sparse mailbox skip everything past the first gap.
- **Persist cursor after every successful chunk** — the cursor was only saved at the end of `sync_folder`, so any mid-backfill error (e.g. async-imap "io: bytes remaining in stream") threw away all progress. Now `save_folder_sync_state` runs inside the chunk loop; a transient error resumes from the last successful chunk instead of from the pre-sync cursor.
- **Lightweight list queries are a 13× speedup** — `listMessagesPaged` was `SELECT *` pulling `body_html` (avg 80 KB/row). For 100 rows that's 8 MB of HTML crossing the IPC bridge for content the list view never renders. Measured SQLite cold-cache timing: `SELECT *` 1.95 s vs selected-columns-only 0.15 s. Add a `lightweight?: boolean` option to `ListMessagesOptions`; when true, SELECT omits `body` and `body_html`, projected via `rowToMessageLight` (same Message shape, body='' and bodyHtml=null). Detail views still use `getMessage(id)` which does a full SELECT for one row. `app/src/stores/data.ts`.
- **Each user-visible section needs its own paginated resource** — the old Imbox used one paginated query and split loaded rows client-side into "New for you" / "Previously seen". With 1000+ messages, anything older than the first 100 newest was invisible to the "Previously seen" section (user couldn't find what they'd already read). Now each section has its own paginated resource (`unreadOnly: true` vs `readOnly: true`) so users can scroll back through thousands of read messages.
- **Tabs for high-density list views** — when a section can have hundreds/thousands of items, having independent paginated tabs is better than one scrollable list with sections. Each tab keeps its own scroll position via KeepAlive. `ImboxTabs` pill-button component with badge counts.
- **Date-grouped section anchors at scale** — for lists with hundreds of items, section headers (今天 / 昨天 / 本周早些 / 本月早些 / 1月) make navigation far easier than scrolling. Pure `dateBucket()` + `bucketLabel()` in `app/src/utils/date.ts` with 13 unit tests is the right shape.
- **Per-message actions: call `removeByIds` on BOTH paginated resources** — when an action could match either tab (new or seen), call `removeByIds` on both. `removeByIds` is a no-op when the id isn't present in the loaded window, so it's safe without needing to know which tab the user was on. `app/src/views/Imbox.tsx` `removeFromBoth` helper.
- **Aggregate `error()` accessor for multi-resource hooks** — when a hook (e.g. `useAgent`) owns multiple `createResource` calls, expose one `error()` accessor that returns the first non-undefined error via `??` chaining. The consuming view renders ONE `ErrorState` instead of checking 5 resources.
- **Show-wrap pattern for error states in complex views** — for views with 14+ Show blocks nested across multiple return statements (Calendar, Gate, ScreenerHistory, Insights, Agent, PileBoard), the minimal-invasive way to add error handling is to wrap everything inside the outer `<div>` in `<Show when={!resource.error} fallback={<ErrorState retry={refetch}/>}>`. Close the wrapping `</Show>` right before the outer `</div>`. Doesn't touch any of the data flow / nested Show blocks / existing skeleton or empty sub-components.
- **Mark brand consistency** — topbar using a stock Phosphor `ph-leaf` while splash / full logo / Tauri bundle icons all shared a bespoke paper-plane + palm-leaf SVG is a visible identity gap. `BrandMark` now uses `/src/assets/logo-mark.svg` inline (Vite bundles the asset, no runtime fetch). Removes the unused Icon import.
- **No "M7 实装" placeholders** — buttons whose only effect is `showToast({ message: '导出为 CSV（M7 实装）' })` are worse than not having the button. The user clicked it expecting CSV; they got a toast admitting the action is unwired. AGENTS §3.2 forbids "we'll fix it later" markers. Delete the button.
- **`<img onerror="this.style.display='none'">` on splash assets** — if the splash `<img>` fails to fetch (offline first boot, asset hash mismatch), the rest of the splash still renders. Otherwise the user sees an empty centred text + pulsing dot.
- **Duplicate CSS rule is dead code** — the `body.app-ready #splash { opacity: 0; pointer-events: none }` rule appeared twice in `index.html`'s inline `<style>`. First wins; second is dead per AGENTS §3.2.

### 11.4 Lessons learned (Session 2026-08-19 — v2 Phase 1: Imbox H1, drag wiring, Read Together)

Three user-reported bugs collapsed into two root causes and one reusable pattern.

- **HTML5 native drag + framework-signal drag need both channels wired** — the Tauri Imbox set `dataTransfer.setData("text/plain", m.id)` in `onDragStart` but never called `startDrag()` from the Solid-signal drag context in `app/src/utils/drag.ts`. The `DropBar` reads from that signal via `<Show when={drag().active}>`, so the bar never appeared when the user dragged a message — drag silently did nothing. Lesson: when a feature uses two parallel transport channels (native browser event + framework signal), every drag-start handler must write to BOTH. Add a smoke test that asserts the DropBar mounts when a card is dragged; the absence of this test let the regression ship.
- **DropBar must enumerate all drop targets, not just buckets** — the v10 prototype's drop bar has 8 zones (5 buckets + 3 workflow piles: pending / saved / remind). The Tauri `DropBar.tsx` only rendered 5 bucket buttons, so the workflow targets were unreachable from the drag surface even though the per-message action menu and the `l` / `a` / `z` keyboard shortcuts wired the same flags. Lesson: when porting from a prototype, copy the full target matrix, not a subset. Extend the `DragTarget` union type to `MessageBucket | "pending" | "saved" | "remind"` and dispatch in the commit handler — workflow targets call the existing `replyLater` / `setAside` actions (or set `bubbleUpAt` to tomorrow 9am for remind), bucket targets call `moveMessageToBucket`. `app/src/utils/drag.ts`, `app/src/components/DropBar.tsx`, `app/src/views/Imbox.tsx`.
- **Default remind time = tomorrow 9am local** — `message:bubble-up` shortcut already uses `tomorrow.setHours(9, 0, 0, 0)`. The drag-to-Remind path now uses the same default. If users need a different time, they can adjust in PileBoard → Remind. This avoids opening a datetime picker mid-drag (jarring UX).
- **DropBar's commit must call `endDrag` in `finally`** — a commit handler that throws (network blip, DB lock) without clearing the signal would leave the DropBar permanently visible. Wrap the `await c(target)` in `try { ... } finally { endDrag(); }` so the bar closes regardless of commit success.
- **Big page title ≠ prototype's empty-state text** — the v10 prototype's `renderImbox()` doesn't render an `<h1>Imbox</h1>` at all; the title is implicit (topbar nav + empty-state copy). The Tauri Imbox added a small inline H1 at `--text-h3` (20-26px) cramped in the same flex row as counts, sort badge, filter, and sync buttons. User called it "too far left, not aligned." Lesson: when the prototype is silent on a UI element, don't infer "small inline label" — promote to a real `--text-h1` (32-44px) heading on its own block line. Counts and controls move to a second row. `app/src/views/Imbox.tsx` `ImboxHeader`. Three sibling views (Agent, FocusReply, PileBoard) use the same `--text-h3` pattern; same fix can be applied per view.
- **One-at-a-time reader needs lightweight list + per-message body fetch** — `ReadTogether` called the full `listMessages()` which pulled every `body` and `body_html` in the table just to render one card. Switch the list to `listMessagesPaged({ bucket: 'imbox', direction: 'in', unreadOnly: true, lightweight: true })` (already in use by Imbox since 8/18) and fetch the current message's full row lazily via `getMessage(currentId)`. The lightweight row is still available as a fallback while the body fetch is in flight. `app/src/views/ReadTogether.tsx`.
- **Prefetch the next body on advance** — after the user clicks Next, the body fetch for the new current is on the critical path. A `createEffect(() => { getMessage(unread()[index()+1]?.id) })` runs in the background and the next click hits the SQLite cache. Cuts the perceived wait between cards.
- **Iframe srcdoc needs `sandbox=""` (empty, not `allow-same-origin`)** — `htmlEmailSrcdoc` is reused in ReadTogether. With `sandbox=""` the iframe is fully sandboxed: no scripts, no same-origin, no top navigation. The click interceptor inside the srcdoc posts a message to the parent which calls `openUrl()` in the OS browser. Same pattern as MessagePanel. Without sandbox the srcdoc's inline script could reach the host's localStorage and exfiltrate the user's data on hostile email.
- **Iframe auto-size on load** — read `iframe.contentDocument.body.scrollHeight` in `onLoad` and set `iframe.style.height = scrollHeight + 24px`. Long emails no longer need a nested scrollbar inside the card. If the body fetch throws (cross-origin or sandboxed) the catch block leaves the `min-height: 240px` from the style attribute; the card stays readable.

### 11.6 Lessons learned (Session 2026-08-19 — `htmlEmailSrcdoc` defer, perf audit, P2 backlog)

- **Synchronous DOMPurify in a render expression blocks the main thread on click path** — `htmlEmailSrcdoc` calls DOMPurify inline; for an 80 KB body_html (Feishu average per §10.5) the parse + sanitize + DOM walk is 200-600 ms on a single thread, freezing the UI even though the panel chrome could paint instantly. Fix: drive the iframe's `srcdoc` from a writable signal, schedule the sanitize via `setTimeout(0)` in a `createEffect`, and use a stale-closure counter to discard sanitzes for messages the user already moved past. `app/src/panels/MessagePanel.tsx`, `app/src/views/ReadTogether.tsx`. Net effect: click → panel paints in one frame → iframe populates 100-400 ms later without blocking scroll / next-click.
- **Defer + signal, not IIFE** — the old `srcdoc={(() => { const doc = htmlEmailSrcdoc(...); return doc; })()}` runs the sanitize every time the render re-evaluates. Driving the attribute from a signal + an effect means the work happens exactly once per bodyHtml change, and Solid's reactivity takes care of re-applying the attribute when the signal updates.
- **Static audit before guessing on remaining perf** — when the user reported "still slow after the pool fix", a 30-minute `grep` audit of `app/src/` (looking for `setInterval` / `createResource(listX)` / `htmlEmailSrcdoc` callers / `console.time` patterns) found the real culprits faster than guessing: (a) the DOMPurify synchronous call in 3 view files (now fixed), (b) `MessagePanel` mounting 5 full-table queries on every message view, (c) `Topbar` polling every 10 s for state that has a push event already. Recorded in `docs/PERF-AUDIT-2026-08-19.md` as the P2 backlog. Profile-first beats speculation.

### 11.7 Lessons learned (Session 2026-08-20 — 8 GB boot OOM from full-table pulls)

**This is a critical lesson. Read it before touching `bootstrap.ts` or any `*Loop` / `tick()` function.**

- **NEVER pull a full table across the IPC bridge in `bootstrap.ts` or any interval-tick function.** The first .dmg I built (2026-08-20 15:11) spiked to **8 GB RAM on first open** and locked up the Mac. Root cause was two full-table pulls running on app boot, in parallel:

  1. `src/bootstrap.ts` line 119 ran a `Promise.all([listAccounts(), listContacts(), listMessages(), listFiles(), listEvents(), listTasks(), listDrafts(), listAgentSessions(), listAgentTasks(), listAgentDrafts(), listAgentAudit(), listNotifications(), listSnippets(), listStickies(), listContactNotes(), listClips(), listFollowUps(), listScheduledSends(), listLabels(), listShortcuts(), listBundleConfigs()])`. The results were **ignored** — every view that needs any of them calls `listX()` itself in `createResource` when the user navigates to it. The bootstrap was warming nothing; it was just torching the heap. `listMessages()` alone returned 4,076 rows × 90 KB `body_html` average = 360 MB. Through the IPC bridge + `JSON.parse` + V8 garbage + a few 5 MB-row outliers, peak was 8 GB before the user could click anything.
  2. `src/services/reminder.ts` line 27 + 71: the 60 s reminder tick called `listMessages()` and `listFollowUps()` and filtered in JS. Same memory hit, every minute, on a hot path.

  **The fix is two new scoped queries** in `src/stores/data.ts`:
  - `listMessagesForReminder(nowIso)` — `SELECT id, subj, bucket, unread, bubble_up_at FROM messages WHERE bubble_up_at IS NOT NULL AND bubble_up_at <= $1 AND bucket = 'imbox' AND unread = 0 ORDER BY bubble_up_at ASC LIMIT 50`. Returns at most 50 rows, no body. Structural assertion in the test confirms `body` and `bodyHtml` are NOT in the projection.
  - `listFollowUpsDue(nowIso)` — same shape for follow-ups, no body. Returns at most 100 rows.

  `bootstrap.ts` was trimmed to the only 4 small lookups the topbar / sidebar / global keyboard handler need at boot: `listAccounts`, `listLabels`, `listShortcuts`, `listBundleConfigs`. Every other list is lazy via `createResource` on view mount.

- **Rule of thumb for any function called from `*Loop` / `tick()` / `bootstrap.ts` / `App.tsx` mount**: the IPC payload must be **bounded** — under 10 KB for a 4,000-row account. Either pre-filter in SQL (with `LIMIT` + `WHERE`) or read the full row by `id` only when the user actually opens it. If a "background" function would have called `listX()` to find 0-3 rows, replace it with a scoped query that returns the 0-3 rows directly.

- **The `Promise.all` at app boot is a smell.** Every list call there is one the user has not asked for yet. If a view needs the data, it has its own `createResource` and will fire on mount. The boot `Promise.all` is usually a leftover from a prototype that used a single global store; in SendPalm the per-view `createResource` is the right pattern. Boot `Promise.all` should be reserved for genuinely app-wide data (accounts, labels, shortcuts) and even then, every member of the array should be small (no `body_html`, no per-row message arrays).

- **A full-table `SELECT *` on a 4,000-row SQLite table is not the slow part. The slow part is the IPC round-trip + `JSON.parse` of the result.** With `body_html` averaging 90 KB (and outliers up to 5 MB per row), 4,000 rows = 360 MB. The SQLite read is sub-second; the JS-side materialization is what kills the heap. Always measure with `chrome://inspect` / V8 heap snapshot before assuming the database is the bottleneck.

- **Verification protocol for any new "background" or boot-time function**: (a) write a test that asserts the return type's structural shape (e.g. `expect(Object.keys(row).sort()).toEqual(['bucket', 'id', 'subj', 'unread'].sort())` for a 5-key projection), (b) assert the `LIMIT` actually fires, (c) assert the `WHERE` filter excludes the rows it should (not-yet-due, already-done, wrong bucket). A naked `listX()` call with no test is a regression risk.

- **9 new unit tests** for the two scoped queries cover: structural shape (no `body` / `bodyHtml` in the slice), future `bubbleUpAt` excluded, already-unread excluded, non-`imbox` bucket excluded, `LIMIT 50` enforced, `status='pending'` only, `surfacedAt IS NULL` only, `dueAt <= now` only. Total vitest 263 → 272.
- **Branch deletion loses work silently** — `feat/perf-db-pool` was deleted via `git branch -d` after being implicitly assumed merged. It wasn't — the `merge` of `feat/v2-phase1-ship-it` to main went through, but the perf-db-pool pool fix + §11.5 docs never made it. Always `git push <remote> <branch>` before `git branch -d`, or verify `git log main..deleted-branch --oneline` is empty. (Recovered via `git reflog` + `git reset --hard` in this session.)
- **`pnpm tauri dev` + a second `cargo` invocation in another project will deadlock on the shared `~/.cargo/shared-target/` lock** — both projects share the same `target/` dir by default, so a Tauri iOS-sim build in another project can block the macOS debug build here. Either wait, kill the other build, or set `CARGO_TARGET_DIR` on the second project to a separate dir. Cost: ~1 GB extra disk per separate target.

### 11.8 Lessons learned (Session 2026-08-20 — Tofu icons, 404 logo, slow scroll, weird HTML body)

User reported four symptoms in one screenshot: sidebar shows `※1`/`※2`/… (tofu), the topbar SendPalm logo is a broken box, opening a message lags, and the rendered email body shows raw `[ Stripe ][https://stripe.com…]` instead of clickable links. All four trace to two structural mistakes: assets not surviving the move to Tauri 2's strict CSP, and literal asset paths in TSX not surviving the move to Vite's production build.

- **Any external `<script src>` or `<link href>` is a future Tauri-CSP regression** — `app/index.html` loaded Phosphor from `https://unpkg.com/@phosphor-icons/web@2.1.1`. Tauri 2's `app.security.csp` in `tauri.conf.json` is `script-src 'self' 'unsafe-inline'` by design (and rightly so). The CDN script was silently blocked, the Phosphor webfont never loaded, and every `<i class="ph ph-tray">` rendered as a tofu. **Rule: anything the app needs at render time must be an npm dependency that Vite bundles locally.** The fix was `pnpm add @phosphor-icons/web` + `import "@phosphor-icons/web/regular"` in `src/index.tsx`. Build emits `Phosphor-*.woff2` (147 KB) + `.ph.ph-tray:before { content: "\eXXX" }` rules in the bundled CSS. The same rule applies to any Google Font, CDN-hosted polyfill, jsDelivr `esm.sh` script, etc. Verify by running `pnpm build && grep -c "phosphor" dist/index.html` — the answer should be 0.

- **Literal asset paths in JSX are not processed by Vite** — `BrandMark.tsx` used `src="/src/assets/logo-mark.svg"` as a string literal. Vite only rewrites asset paths that go through the import pipeline; literal strings inside `.tsx` files are NOT scanned. In dev, Vite serves `/src/assets/...` from a virtual path so it works. In production, the file is at `dist/assets/logo-<hash>.svg` and the literal string 404s. **Rule: in TSX/JSX, every local asset must be imported.** `import logoUrl from "/src/assets/x.svg?url"` then `src={logoUrl}`. The `?url` query is Vite's opt-in for "treat this as a URL only, don't inline the contents" (use `?raw` for text, `?inline` for base64). `index.html` is the ONE exception: Vite scans it for `<img src>`, `<link href>`, and `<script src>` and rewrites them automatically. Everything inside a `.tsx` file must go through the import system.

- **`<i class="ph ph-tray">` without the Phosphor CSS = silent tofu** — the bug surfaces only in production. In dev, the CDN script "kinda works" (Chromium may have cached the file from a prior session, or the system font happens to have a PUA glyph at the codepoint). In production with the CSP blocking, the failure is total and renders every icon as a missing-glyph box. Add a Playwright smoke test that asserts at least one `<i class="ph">` has a non-zero `getBoundingClientRect().width` after the bundle loads — that would have caught this.

- **Multi-iframe resize storm** — `MessagePanel` mounts one iframe per expanded thread message (3-5 in practice). The original iframe `ref` callback ran a 30-frame `requestAnimationFrame` resize loop, 3 `setTimeout` resizes (300/1200/3000 ms), AND a `ResizeObserver` on the body. With 5 iframes that's ~150 forced reflows per message open, all synchronously scheduled on the same frame. The user felt it as "scroll feels laggy" because the scroll container's compositor was also busy processing the reflows. **Rule: one `ResizeObserver` on the iframe body, registered on `load`, is enough.** It fires for every content change (font load, image reveal, dynamic DOM mutation) without forcing the main thread to drain a queue. Drop RAF loops, drop setTimeout resizes, keep ResizeObserver.

- **Always-on `transform: translateY(0)` on a scroll container hurts scroll** — the MessagePanel scroll container had `transform: "translateY(0)"` and `transition: "transform 0.42s cubic-bezier(...)"` applied at all times (only the pull-to-refresh gesture changed the value). The browser saw "this element has a transform transition" and promoted it to its own compositor layer. Native scroll then had to round-trip through the compositor, which on dense bodies (8 MB `body_html`, large inline images) was visible as jank. **Rule: only apply `transform` while an animation is actually playing.** When no animation is active, leave the property undefined (not `translateY(0)`). The CSS `transition` declaration should be paired with the actual `transform` change, not a constant no-op.

- **`plainTextToHtml` previously only linkified bare `https?://…` URLs** — Stripe (and most ESPs) ship a plain-text alternative in the form `[ Label ][https://url?utm=…]` so screen readers announce the destination. The old converter left both bracket groups as literal text. **Rule: any text-to-HTML helper that's used in a body-fallback path should know the common email plain-text dialects.** The fix is a single-pass alternation regex: `[label][url]` → reference link, `[label](url)` → inline link, bare `https?://…` / `mailto:…` → fallback. Use single-pass `String.prototype.replace` with a callback that picks the matching alternative; sequential `replace().replace().replace()` will re-match URLs it just inserted into `href="…"`.

- **DOMPurify `sanitize` is synchronous and slow on large bodies** — already captured in §11.6, but the symptom in this session was the same "feels laggy" complaint. If you find yourself adding more resize observers or RAF loops to a panel that mounts an iframe, the real fix is almost always: defer the sanitize to a `setTimeout(0)` effect, drive the iframe `srcdoc` from a writable signal, and discard stale sanitzes via a counter.

### 11.9 Lessons learned (Session 2026-08-20 — code-splitting, shell-first render, no `ready()` gate)

User reported the app still felt slow after the icon/asset fixes (§11.8), with two distinct symptoms: (a) the splash sits for a second on first open before anything else paints, and (b) the main list view is laggy. Both had the same root cause family: the top-level `App` was blocking paint on a `ready()` signal that was waiting for boot-time IPC, and every view file was bundled into the initial JS even though only one is ever visible at a time.

- **No `ready()` gate on the top-level shell.** `App.tsx` previously did `<Show when={ready() && !initError()}>...full body...</Show>` where `ready` was set after `initApp()` resolved (4 IPC round-trips: `load(STORE_PATH)`, `ensureDefaultShortcuts`, then `Promise.all([listAccounts(), listLabels(), listShortcuts(), listBundleConfigs()])`). The user saw only the splash for the full bootstrap duration even though every component in the body already does its own `createResource` and doesn't need any of that data to render. **Rule: the shell paints on the first frame. Every component pulls its own data via `createResource` and shows a per-view skeleton while the resource resolves. The boot-time `initApp` keeps running in the background and only mutates stores (settings, agent memory, onboarding gating) that the shell doesn't read until the user actually navigates to a view that needs them.** The only fatal case that still gates the shell is a thrown `initError()` — surfaced as a full-screen banner. Splash hide is now `requestAnimationFrame × 2` so the browser commits the first layout before the splash starts clearing (single rAF would produce a one-frame flash of paper-coloured void).

- **Lazy-load every view via `lazy(() => import(...).then(m => ({ default: m.X })))` + `<Suspense>`.** `Main.tsx` previously did `import { Imbox } from "../views/Imbox"`, `import { Calendar } from "../views/Calendar"`, etc. for all 22 views. Calendar (2264 lines), Imbox (1835), Settings (2214), Agent, Insights, etc. all had to be parsed by the V8 engine before the user could click anything. Replacing the static imports with `lazy()` cut the initial bundle from **633 KB → 399 KB raw / 120 KB gzipped**. The active view's chunk downloads after the shell paints; the rest load on demand. Pair with `<Suspense fallback={<FeedSkeleton />}>` so the user sees a skeleton during the chunk fetch. **Rule: anything in `views/*` is reachable via the navigation Switch, not on the initial mount. Static imports defeat the whole point.** If you find yourself adding a new view, also add a `lazy()` line in `Main.tsx` — there is a Vitest case (`Main.test.ts`) that fails the build if a `views/*` import sneaks back in.

- **`<For>` callback gives you `index` for free; never mutate items to attach indices.** `Imbox.tsx` had `Object.assign(item, { _flatIdx: flatIdx })` inside the date-bucket group loop so the per-row `index` prop could carry a global cursor position for j/k navigation. That mutated the SolidJS-tracked message objects, so any `createMemo` reading the items (priority sort, bundle grouping, date bucket) saw a different identity on every regroup and the `<For>` would tear down every row. **Rule: if you need a cross-row index, carry it in a parallel structure (a `startIdx` per group) and let the per-row `<For>` add the local offset.** Mutating tracked state is the single most common SolidJS perf bug; the compiler doesn't catch it because TypeScript happily lets you mutate anything.

- **Match the skeleton to the real layout shape, not the average one.** `Main.tsx`'s `FeedSkeleton` used to render 6 rows; the real Imbox renders up to 100. The user saw a tiny skeleton that "jumped" to 100 rows when the chunk resolved. Bumped to 12 rows with avatar + 2 text lines + a 32px H1 placeholder. 12 rows covers the average viewport without bloating the DOM; 6 is the lower bound, but anything < 8 will visibly shift on first paint.

- **Don't use animated `width` on progress bars.** A naive "X / Y" progress bar that animates `width: 80%` triggers reflow on every frame. Use `transform: scaleX(0.8)` instead — the bar lives on its own compositor layer and the fill animation runs on the GPU. The `<LoadedProgressBar>` in `Imbox.tsx` uses this pattern: `transform: scaleX(${pct() / 100})` + `transform-origin: left center`.

- **The "loading…" vs "未连接" distinction matters in resource-pending state.** A `createResource` that hasn't resolved yet shows `undefined` from its accessor. UI code that does `accounts() ?? []` will report zero items, which looks identical to "no accounts configured". For the `SyncBadge` in `Topbar.tsx` this meant the topbar showed `未连接` for a second on first paint, then suddenly flipped to `1 账户`. The fix: `accounts.loading` is a property on the resource accessor; check it and show `加载中…` before falling through to the empty/non-empty branch.

### 11.10 Lessons learned (Session 2026-08-20 — ResizeObserver leak, MessagePanel loading UX, plain-text streaming)

User reported the app still got slow after a while of use, and that opening a message had no loading state. Two distinct issues:

- **ResizeObserver created in a ref callback with no teardown path.** `MessagePanel.tsx` did `const ro = new ResizeObserver(resize)` inside the iframe ref callback, but never disconnected it. Every time the user opened a new message the `<For>` loop dropped the previous iframe; SolidJS calls the ref callback with `null` on unmount, but the old code didn't handle that case and the observer kept observing the (now-detached) iframe body. After 100 message opens, 100 ResizeObservers + 100 iframe-body references were pinned in the closure scope, and the user felt it as "uses the app for a while, then it gets slow". **Rule: any resource created in a ref callback (observer, listener, interval, RAF handle) needs an explicit teardown path.** The `null` ref callback is the right place to disconnect, plus a top-level `onCleanup` as a safety net for whole-component disposal. Tracked via a module-level `let` (not closure-scoped) so the unmount path can reach the observer instance.

- **No loading state for message detail — the user stares at an empty box for up to a second.** The flow: user clicks a message → MessagePanel mounts → `message()` resource fetches (200-400ms for 80KB body_html) → render. During fetch the panel is empty (only the header bar shows). After fetch, `iframeSrc` is set via `setTimeout(0)` to defer the 200-600ms DOMPurify sanitize. Total: up to 1s of empty space below the header. **Rule: any async work between mount and content needs a visible skeleton, not a blank space.** Added a `MessagePanelSkeleton` that mirrors the real layout (avatar + 2 text lines + body block + "正在加载邮件…" caption). Skeleton component already has a 1.4s shimmer animation; no new CSS needed.

- **Plain-text streaming as perceived-speed accelerator.** Even with the skeleton, the user is reading 0 → 1 message. The plain text body (no sanitize, no iframe) is available immediately as `m.body`. The fix: while `iframeSrc` is still empty for the current message, render `plainTextToHtml(m.body)` as a fallback above the iframe. Perceived speed: 0 → text in <50ms (no sanitize, no iframe), then text swaps to rich HTML when sanitize completes 200-600ms later. The text and the iframe carry the same content (DOMPurify is lossless apart from iframe-specific rendering like tables / images), so the user reads the same words the whole time. Tradeoff: a brief content "swap" on the same line — for text-only emails the swap is invisible; for HTML-heavy emails the user sees a flicker. The `iframeReady` Set tracks per-message sanitize completion so the fallback only fires for the current message.
...
- **WAL journal mode is necessary but not sufficient for read-during-write** — `PRAGMA journal_mode=WAL` lets a *separate SQLite session* read concurrently with a writer. But the sqlx `SqlitePool` defaults to `max_connections(1)`, so all queries share one connection. With one connection, the IMAP sync loop's write transaction (a 200-UID chunk taking seconds on the Feishu account) holds the only connection, and every frontend IPC read blocks on the same connection — even though WAL would have allowed a parallel reader. Fix: `SqlitePoolOptions::new().max_connections(8).connect_with(opts)`. Sync keeps one, the rest are free for IPC handlers. WAL is preserved by setting it on `SqliteConnectOptions` before `connect_with`. `app/src-tauri/src/services/sync_loop.rs::open_pool`.
- **Default sqlx pool size is 1, not the OS default** — for any Tauri app with background writers and concurrent IPC readers, set `max_connections` explicitly. 8 is a reasonable starting point (matches the typical web-server pool size); bump up if IPC handlers start queuing.
- **Set `busy_timeout` on the connect options** — default is 0 (immediate `SQLITE_BUSY` failure). The rare writer-writer collision (e.g. main sync + a newly-inserted account) returns an error even though one would have released its lock milliseconds later. `.busy_timeout(Duration::from_secs(5))` makes the connection wait up to 5 s for the lock.
- **CJK text + digits: no inter-glyph space** — sibling `<span>`s with no gap render "本月早些5" with the count glued to the title. Either (a) wrap in a flex container with `gap: var(--space-2)` and add a visual separator (a pill badge, a `·`, a non-breaking space), or (b) add a CSS `letter-spacing` rule. We picked (a) for the date-group count and bumped `ImboxTabButton`'s gap from 8px to `var(--space-2)` for visual consistency. `app/src/views/Imbox.tsx` `DateGroupedList`.
- **Profile before fix** — adding `console.time` / `console.timeEnd` markers at suspect code paths (Topbar polls, MessagePanel getMessage, htmlEmailSrcdoc build) revealed the real culprit (Rust-side pool contention) in one screenshot of the dev console. Always profile before guessing. The markers are temporary debug; remove them once the data is in.
- **`pnpm tauri dev` + a second `cargo` invocation in another project will deadlock on the shared `~/.cargo/shared-target/` lock** — both projects share the same `target/` dir by default, so a Tauri iOS-sim build in another project can block the macOS debug build here. Either wait, kill the other build, or set `CARGO_TARGET_DIR` on the second project to a separate dir. Cost: ~1 GB extra disk per separate target.
