...
const html = fs.readFileSync(path.join(root, 'prototype-v8.html'), 'utf8');
...
function loadApp() {
  ...
  const dom = new JSDOM(cleanHtml, {
    url: 'file://' + path.join(root, 'prototype-v8.html'),
  ...
  const appScript = fs.readFileSync(path.join(root, 'js/prototype-v8.js'), 'utf8');
...
async function run() {
  ...
  if (!panel.classList.contains('open')) throw new Error('Detail panel did not open');
  ...
  if (meetingCards.length > 0) {
    ...
    await wait(50);
    if (!panel.classList.contains('open')) throw new Error('Meeting panel did not open');
  ...
  personCards[0].click();
  await wait(50);
  if (!panel.classList.contains('open')) throw new Error('Contact panel did not open');
