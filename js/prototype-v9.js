(function() {
  ...
  function emptyStateArt(artClass) {
    const svgs = {
      'art-purple': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 180 180"><circle cx="90" cy="90" r="70" fill="#5856d6" fill-opacity="0.12"/><rect x="50" y="75" width="80" height="55" rx="10" fill="none" stroke="#5856d6" stroke-width="3" stroke-opacity="0.5"/><path d="M50 82 L90 112 L130 82" fill="none" stroke="#5856d6" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" stroke-opacity="0.5"/><path d="M90 42 L95 58 L112 58 L99 68 L104 85 L90 75 L76 85 L81 68 L68 58 L85 58 Z" fill="#5856d6" fill-opacity="0.7"/></svg>',
      'art-green': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 180 180"><circle cx="90" cy="90" r="70" fill="#30b94d" fill-opacity="0.12"/><rect x="52" y="88" width="76" height="48" rx="10" fill="none" stroke="#30b94d" stroke-width="3" stroke-opacity="0.5"/><rect x="58" y="78" width="76" height="48" rx="10" fill="none" stroke="#30b94d" stroke-width="3" stroke-opacity="0.4"/><rect x="64" y="68" width="76" height="48" rx="10" fill="none" stroke="#30b94d" stroke-width="3" stroke-opacity="0.3"/><line x1="76" y1="80" x2="124" y2="80" stroke="#30b94d" stroke-width="2" stroke-opacity="0.4"/><line x1="76" y1="92" x2="118" y2="92" stroke="#30b94d" stroke-width="2" stroke-opacity="0.4"/></svg>',
      'art-orange': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 180 180"><circle cx="90" cy="90" r="70" fill="#f5a623" fill-opacity="0.12"/><path d="M50 78 L50 128 Q50 134 56 134 L124 134 Q130 134 130 128 L130 88 Q130 82 124 82 L96 82 L86 72 L56 72 Q50 72 50 78 Z" fill="none" stroke="#f5a623" stroke-width="3" stroke-opacity="0.5"/><line x1="66" y1="96" x2="114" y2="96" stroke="#f5a623" stroke-width="2" stroke-opacity="0.4"/><line x1="66" y1="110" x2="108" y2="110" stroke="#f5a623" stroke-width="2" stroke-opacity="0.4"/><line x1="66" y1="124" x2="118" y2="124" stroke="#f5a623" stroke-width="2" stroke-opacity="0.4"/></svg>'
    };
  ...
  function bubbleUpMessage(m, duration) {
    ...
    showToast('Reminded until ' + until.toLocaleDateString());
    renderMain();
  }
  ...
  function renderMain() {
    ...
    header.appendChild(el('h1', 'view-title', viewTitle(state.view)));
    if (state.view === 'imbox' || bucketViews.includes(state.view)) {
      const actionLabel = state.view === 'imbox' ? 'Read together' : null;
      if (actionLabel) {
        const action = el('button', 'view-header-action', actionLabel);
        action.addEventListener('click', () => showToast('Read together mode'));
  ...
  function bucketEmptyCopy(view) {
    const map = {
      ...
      screener: { text: '所有新联系人都已经过筛选。你的收件箱由你做主。', title: 'Gate 很干净', art: 'art-purple' },
      ...
      bubbleUp: { text: '被提醒的邮件会在指定时间重新浮上来。', title: 'Remind 为空', art: 'art-purple' },
    };
  ...
  function renderGate() {
    ...
    if (newSenders.length === 0) {
      const empty = renderEmpty('所有新联系人都已经过筛选。你的收件箱由你做主。', 'ph-funnel', 'Gate 很干净', 'art-purple');
  ...
  function renderStream() {
    ...
    allEvents.forEach(ev => {
      ...
      header.appendChild(renderAvatar(contact, 'stream-avatar', contact ? contact.name[0] : '?'));
  ...
  function renderFocusReply() {
    ...
    if (allEvents.length === 0 || state.focusReplyIndex >= allEvents.length) {
      state.focusReplyOpen = false;
      state.focusReplyIndex = 0;
      renderMain();
      return container;
    ...
    const ev = allEvents[state.focusReplyIndex];
    const m = ev.data;
    const contact = getContact(m.pid);

    const header = el('div', 'focus-reply-header');
    const progress = el('div', 'focus-reply-progress', (state.focusReplyIndex + 1) + ' of ' + allEvents.length);
    ...
    const card = el('div', 'focus-reply-card');
    const meta = el('div', 'focus-reply-meta');
    meta.appendChild(renderAvatar(contact, 'focus-reply-avatar', contact ? contact.name[0] : '?'));
    ...
    card.appendChild(el('h2', 'focus-reply-subject', m.subj));
    ...
    const actions = el('div', 'focus-reply-actions');

    const replyBtn = el('button', 'btn btn-primary btn-lg');
    ...
    doneBtn.addEventListener('click', () => {
      replyLaterMessage(m);
      // replyLaterMessage toggles flag off via clearWorkflowFlags
      state.focusReplyIndex++;
      renderMain();
    });
  ...
  function renderImboxPile(events, title, iconName, pileId) {
    ...
    if (state.expandedPile === pileId) {
      ...
      events.forEach(ev => {
        ...
        const m = ev.data;
        const contact = getContact(m.pid);
        row.appendChild(renderAvatar(contact, 'pile-drawer-avatar', contact ? contact.name[0] : '?'));
      ...
      boardBtn.addEventListener('click', () => setView(pileId === 'pending' ? 'replyLater' : 'setAside'));
  ...
  function renderImbox() {
    ...
    if (replyLater.length || setAside.length) {
      ...
      if (replyLater.length) piles.appendChild(renderImboxPile(replyLater, 'Pending', 'ph-clock', 'pending'));
      if (setAside.length) piles.appendChild(renderImboxPile(setAside, 'Saved', 'ph-push-pin', 'saved'));
  ...
  function renderContactPanel(c) {
    ...
    const info = el('div', 'panel-info contact-header-info');
    info.appendChild(el('div', 'panel-name', c.name));
    info.appendChild(el('div', 'panel-role', c.tl + ' · ' + c.co));

    const badges = el('div', 'contact-header-badges');
    const healthBadge = el('span', 'contact-health-badge');
    healthBadge.style.background = hexToRgba(c.scC || '#888888', 0.12);
    healthBadge.style.color = c.scC || 'var(--text-secondary)';
    healthBadge.appendChild(el('span', '', c.scL || 'Health'));
    healthBadge.appendChild(el('span', 'contact-health-score', c.sc));
    ...
    const callBtn = el('button', 'btn btn-secondary btn-sm btn-icon');
    callBtn.title = 'Call';
    callBtn.appendChild(icon('ph-phone'));
    callBtn.addEventListener('click', () => showToast('Calling ' + c.ph));

    const copyBtn = el('button', 'btn btn-secondary btn-sm btn-icon');
    copyBtn.title = 'Copy email';
    copyBtn.appendChild(icon('ph-copy'));
    copyBtn.addEventListener('click', () => {
      if (navigator.clipboard) navigator.clipboard.writeText(c.em).then(() => showToast('Email copied'));
      else showToast(c.em);
    });
    ...
    const headerActions = el('div', 'panel-actions contact-header-actions');
    ...
    const contactActionBar = el('div', 'contact-action-bar');
    const contactActions = [
      { icon: c.notify !== false ? 'ph-bell' : 'ph-bell-slash', label: c.notify !== false ? 'Notify' : 'Mute', action: () => { c.notify = c.notify === false; renderMain(); showToast(c.notify !== false ? 'Notifications on' : 'Notifications off'); } },
      { icon: 'ph-tray', label: 'Deliver to ' + viewTitle(c.defaultBucket || 'imbox'), action: (btn) => {
        openContextMenuFromElement(btn, [
          { label: 'Inbox', action: () => { c.defaultBucket = 'imbox'; renderMain(); showToast('Deliver to Inbox'); } },
          { label: 'Stream', action: () => { c.defaultBucket = 'feed'; renderMain(); showToast('Deliver to Stream'); } },
          { label: 'Records', action: () => { c.defaultBucket = 'paperTrail'; renderMain(); showToast('Deliver to Records'); } },
        ]);
      } },
      { icon: 'ph-funnel', label: 'Autofile', action: () => showToast('Autofile rules for ' + c.name) },
      { icon: 'ph-note', label: 'Add note', action: () => showToast('Add note to ' + c.name) },
    ];
    contactActions.forEach(cfg => {
      const btn = el('button', 'contact-action-btn');
      btn.appendChild(icon(cfg.icon));
      btn.appendChild(el('span', '', cfg.label));
      btn.addEventListener('click', () => cfg.action(btn));
      contactActionBar.appendChild(btn);
    });
    wrapper.appendChild(contactActionBar);

    const content = el('div', 'panel-content');
    content.appendChild(renderContactStats(c));
    ...
    content.appendChild(renderContactInsights(c));
  ...
  function changeSelectedDate(days) {
    ...
    if (d.getMonth() !== state.calendarMonth.getMonth()) {
  ...
  function renderMiniCalendar(filteredMeetings) {
    const wrap = el('div', 'calendar-month-wrap calendar-mini');
  ...
  function renderDayAgenda(selectedMeetings) {
  ...
  function renderDayTimeline(selectedMeetings) {
    const wrap = el('div', 'calendar-timeline-wrap');
    const header = el('div', 'calendar-timeline-header');
    const title = el('div', 'calendar-timeline-title', formatDateLabel(state.calendarSelected));

    const nav = el('div', 'calendar-timeline-nav');
    const prevDayBtn = el('button', 'icon-btn');
    prevDayBtn.title = 'Previous day';
    prevDayBtn.appendChild(icon('ph-caret-left'));
    prevDayBtn.addEventListener('click', () => changeSelectedDate(-1));

    const nextDayBtn = el('button', 'icon-btn');
    nextDayBtn.title = 'Next day';
    nextDayBtn.appendChild(icon('ph-caret-right'));
    nextDayBtn.addEventListener('click', () => changeSelectedDate(1));

    const strip = el('div', 'calendar-day-strip');
    const center = state.calendarSelected;
    const dayLetters = ['S', 'M', 'T', 'W', 'T', 'F', 'S'];
    const today = new Date(2026, 6, 20);
    for (let offset = -3; offset <= 3; offset++) {
      const d = new Date(center.getFullYear(), center.getMonth(), center.getDate() + offset);
      const isSelected = sameDate(d, center);
      const isToday = sameDate(d, today);
      const dayBtn = el('button', 'strip-day' + (isSelected ? ' selected' : '') + (isToday ? ' today' : ''));
      dayBtn.appendChild(el('span', 'strip-day-name', dayLetters[d.getDay()]));
      dayBtn.appendChild(el('span', 'strip-day-number', d.getDate()));
    ...
    nav.appendChild(prevDayBtn);
    nav.appendChild(strip);
    nav.appendChild(nextDayBtn);
    nav.appendChild(todayBtn);
    header.appendChild(title);
    header.appendChild(nav);
    wrap.appendChild(header);

    const scroll = el('div', 'calendar-timeline-scroll');
    const timeline = el('div', 'calendar-timeline');
    const labels = el('div', 'timeline-hour-labels');
    ...
    for (let h = startHour; h <= endHour; h++) {
      const labelRow = el('div', 'timeline-hour-label', formatMinutes(h * 60));
      labels.appendChild(labelRow);
      const row = el('div', 'timeline-hour-row');
      content.appendChild(row);
    }

    if (sameDate(state.calendarSelected, today)) {
      const appNow = new Date('2026-07-20T09:00:00');
      const nowMinutes = appNow.getHours() * 60 + appNow.getMinutes();
      if (nowMinutes >= startHour * 60 && nowMinutes <= endHour * 60) {
        const line = el('div', 'timeline-current-time');
        line.style.top = ((nowMinutes - startHour * 60) / 60) * hourHeight + 'px';
        content.appendChild(line);
      }
    }

    const layout = layoutDayEvents(selectedMeetings, startHour, endHour);
    layout.forEach(item => {
      const ev = el('div', 'timeline-event');
      ev.style.top = ((item.start - startHour * 60) / 60) * hourHeight + 'px';
      ev.style.height = Math.max(28, (item.duration / 60) * hourHeight) + 'px';
      ev.style.left = (item.col / item.maxCols * 100) + '%';
      ev.style.width = (100 / item.maxCols - 1) + '%';
      ev.appendChild(el('div', 'timeline-event-time', formatMinutes(item.start) + ' - ' + formatMinutes(item.end)));
      ev.appendChild(el('div', 'timeline-event-title', item.m.title));
      ev.appendChild(el('div', 'timeline-event-people', item.m.ppl));
      ev.addEventListener('click', () => openMeeting(item.m));
  ...
  function renderCalendar() {
    ...
    const timeline = renderDayTimeline(selectedMeetings);
    const sidebar = el('div', 'calendar-sidebar');
    sidebar.appendChild(renderMiniCalendar(filteredMeetings));
    sidebar.appendChild(renderDayAgenda(selectedMeetings));
...
`;
  ...
  function renderMessagePanel(m) {
    ...
    function makeActionBtn(iconName, label, primary, iconOnly) {
    ...
    const replyBtn = makeActionBtn('ph-arrow-u-up-left', 'Reply', true);
    const replyLaterBtn = makeActionBtn('ph-clock', 'Pending', false, true);
    const setAsideBtn = makeActionBtn('ph-push-pin', 'Saved', false, true);
    const bubbleUpBtn = makeActionBtn('ph-arrow-fat-line-up', 'Remind', false, true);
    const moreBtn = makeActionBtn('ph-dots-three', 'More', false, true);
  ...
  function renderFeedItem(ev, view, index) {
    if (ev.type === 'message') {
      ...
      if (hasAgentDraft && view === 'imbox') {
        return renderDraftCard(m, contact, index);
      }
      ...
      const subj = el('span', 'feed-subject', m.subj);
      const prev = el('span', 'feed-preview', m.prev);
      bottomRow.appendChild(subj);
      bottomRow.appendChild(el('span', 'feed-preview-divider', ' — '));
      ...
      const actionConfigs = [
        { icon: 'ph-clock', title: 'Pending', action: () => replyLaterMessage(m) },
        { icon: 'ph-push-pin', title: 'Saved', action: () => setAsideMessage(m) },
        { icon: 'ph-arrow-fat-line-up', title: 'Remind', action: () => bubbleUpMessage(m, 'tomorrow') },
        { icon: 'ph-archive', title: 'Archive', action: () => archiveMessage(m) },
      ];
    ...
    if (ev.type === 'meeting') {
      ...
      const body = el('div', 'feed-body');
      const topRow = el('div', 'feed-top-row');
      topRow.appendChild(el('div', 'feed-subject', m.title));
      topRow.appendChild(el('span', 'feed-spacer'));
  ...
  function renderDraftCard(m, contact, index) {
    const draft = findDraftForMessage(m) || { preview: m.prev };
    const isCursor = index === state.cursorIndex;
    const isSelected = state.selectedIds.has(m.id);
    const card = el('div', 'feed-card draft-card' + (isCursor ? ' cursor' : '') + (isSelected ? ' selected' : ''));
    card.dataset.index = index;
...
`;
  ...
  const commands = [
    ...
    { id: 'goImbox', label: 'Go to Inbox', section: 'Views', icon: 'ph-tray', hint: '' },
    { id: 'goFeed', label: 'Go to Stream', section: 'Views', icon: 'ph-newspaper', hint: '' },
    { id: 'goPaperTrail', label: 'Go to Records', section: 'Views', icon: 'ph-receipt', hint: '' },
    { id: 'goScreener', label: 'Go to Gate', section: 'Views', icon: 'ph-funnel', hint: '' },
  ...
  function showContextMenuForMessage(e, m) {
    ...
    const items = [
      ...
      { type: 'divider' },
      { label: 'Pending', icon: 'ph-clock', action: () => replyLaterMessage(m) },
      { label: 'Saved', icon: 'ph-push-pin', action: () => setAsideMessage(m) },
      { label: 'Remind...', icon: 'ph-arrow-fat-line-up', action: () => bubbleUpMessage(m, 'tomorrow') },
      { type: 'divider' },
      { label: 'Move to Inbox', icon: 'ph-tray', action: () => moveMessageToBucket(m, 'imbox') },
      { label: 'Move to Stream', icon: 'ph-newspaper', action: () => moveMessageToBucket(m, 'feed') },
      { label: 'Move to Records', icon: 'ph-receipt', action: () => moveMessageToBucket(m, 'paperTrail') },
      ...
      if (!isTyping && !e.metaKey && !e.ctrlKey && !e.altKey) {
        ...
        if (listViews.includes(state.view) && !isOverlayOpen()) {
          ...
          if (events.length > 0) {
            ...
            } else if (e.key === 'e' || e.key === 'r' || e.key === 'l' || e.key === 's' || e.key === 'b') {
              e.preventDefault();
              if (state.cursorIndex < 0) state.cursorIndex = 0;
              const ev = events[state.cursorIndex];
