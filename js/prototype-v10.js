(function() {
  ...
  function renderPersonCard(c) {
    ...
    const writeBtn = el('button', 'btn btn-primary btn-xs');
  ...
  function renderCalendar() {
    ...
    // HEY-style: clean 2-panel layout
    // LEFT: mini month picker (compact, scrollable)
    // RIGHT: day timeline with selected day's events as colorful cards

    const left = el('div', 'calendar-left');
    const monthPicker = renderMiniCalendar(filteredMeetings);
    left.appendChild(monthPicker);

    container.appendChild(left);

    const right = el('div', 'calendar-right');
    right.appendChild(renderDayTimeline(selectedMeetings));
  ...
  function renderFiles() {
    ...
    const pageHeader = el('div', 'files-page-header');
    const headerLineLeft = el('div', 'files-page-header-line');
    const headerTitle = el('h1', 'files-page-title', 'All Files');
    const headerLineRight = el('div', 'files-page-header-line');
    pageHeader.appendChild(headerLineLeft);
    pageHeader.appendChild(headerTitle);
    pageHeader.appendChild(headerLineRight);
    ...
    files.forEach(f => {
      ...
      const fromCell = el('td', 'file-cell-from');
      if (contact) {
        const fromWrap = el('div', 'file-cell-from-wrap');
        fromWrap.appendChild(renderAvatar(contact, 'file-cell-avatar', contact.name[0], () => openContact(contact.id)));
...
`;
  ...
  function renderComposeWindow(context) {
    ...
    if (mode === 'new') {
      ...
      const autoTitleText = el('span', 'compose-auto-title-text', '');
      autoTitleHint.appendChild(autoTitleText);
      const applyBtn = el('button', 'compose-auto-title-apply', 'Use');
